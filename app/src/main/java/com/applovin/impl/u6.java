...
public abstract class u6 {
    ...
    public static boolean a(String str) {
        ...
        return split.length == 3 && CommonGetHeaderBiddingToken.HB_TOKEN_VERSION.equals(split[0]) && Pattern.matches("(\\d+(\\.\\d+)*)?", split[1]) && split[2].length() >= 3 && split[2].startsWith("dv.") && Pattern.matches("(\\d+(\\.\\d+)*)?", split[2].substring(3));
    }
