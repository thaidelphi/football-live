...
public class Intent {
    ...
    public static void canOpenIntent(JSONObject jSONObject, WebViewCallback webViewCallback) {
        ...
        } catch (IntentException e8) {
            DeviceLog.exception("Couldn't resolve intent", e8);
            webViewCallback.error(e8.getError(), e8.getField());
        }
    }
    ...
    public static void canOpenIntents(JSONArray jSONArray, WebViewCallback webViewCallback) {
        ...
        for (int i10 = 0; i10 < length; i10++) {
            ...
            try {
                jSONObject.put(optJSONObject.optString(ug.f21009x), checkIntentResolvable(intentFromMetadata(optJSONObject)));
            } catch (IntentException e8) {
                DeviceLog.exception("Exception parsing intent", e8);
                webViewCallback.error(e8.getError(), e8.getField());
                return;
    ...
    private static boolean setCategories(android.content.Intent intent, JSONArray jSONArray) {
        ...
        for (int i10 = 0; i10 < jSONArray.length(); i10++) {
            try {
                ...
                DeviceLog.exception("Couldn't parse categories for intent", e8);
                return false;
            }
        }
        return true;
        ...
        if (jSONArray != null) {
            for (int i10 = 0; i10 < jSONArray.length(); i10++) {
                try {
                    ...
                    if (!setExtra(intent, jSONObject.getString(b9.h.W), jSONObject.get(b9.h.X))) {
                        return false;
                    }
                } catch (Exception e8) {
                    DeviceLog.exception("Couldn't parse extras", e8);
                    return false;
                }
            }
            return true;
