...
public class C06309b {
     ...
     */
    public static C06309b A04(File file) throws IOException {
        try {
            ...
            if (file.length() >= A00()) {
                ...
                if (readLong == -360011992771067903L) {
                    ...
                    if (randomAccessFile.length() >= A02((int) readLong2)) {
                        ...
                        return new C06309b(randomAccessFile, jArr, randomAccessFile.length());
                    }
    ...
    public C06309b(RandomAccessFile randomAccessFile, long[] jArr, long j10) throws IOException {
    ...
    public static C06309b A03(File file) throws IOException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 33));
        try {
            randomAccessFile.writeLong(-360011992771067903L);
            randomAccessFile.writeLong(C06319c.A03);
            randomAccessFile.write(new byte[C06319c.A03 * 8]);
            randomAccessFile.getFD().sync();
            return new C06309b(randomAccessFile, new long[C06319c.A03], randomAccessFile.length());
        } catch (IOException unused) {
