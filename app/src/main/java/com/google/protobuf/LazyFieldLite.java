...
public class LazyFieldLite {
    ...
    void writeTo(Writer writer, int i10) throws IOException {
        if (this.memoizedBytes != null) {
            writer.j(i10, this.memoizedBytes);
            return;
        }
        ByteString byteString = this.delayedBytes;
        if (byteString != null) {
            writer.j(i10, byteString);
        } else if (this.value != null) {
            writer.z(i10, this.value);
        } else {
            writer.j(i10, ByteString.EMPTY);
        }
    }
