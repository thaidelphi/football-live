...
public final class UnknownFieldSetLite {
    ...
    private static void writeField(int i10, Object obj, Writer writer) throws IOException {
        ...
        if (tagWireType == 0) {
            ...
            writer.D(tagFieldNumber);
            ((UnknownFieldSetLite) obj).writeTo(writer);
            writer.J(tagFieldNumber);
        } else {
            writer.J(tagFieldNumber);
            ((UnknownFieldSetLite) obj).writeTo(writer);
            writer.D(tagFieldNumber);
    ...
    public final void printWithIndent(StringBuilder sb, int i10) {
        for (int i11 = 0; i11 < this.count; i11++) {
            y.d(sb, i10, String.valueOf(WireFormat.getTagFieldNumber(this.tags[i11])), this.objects[i11]);
        }
    }
    ...
    public void writeAsMessageSetTo(Writer writer) throws IOException {
        if (writer.y() == Writer.FieldOrder.DESCENDING) {
            for (int i10 = this.count - 1; i10 >= 0; i10--) {
                writer.c(WireFormat.getTagFieldNumber(this.tags[i10]), this.objects[i10]);
            }
            return;
        }
        for (int i11 = 0; i11 < this.count; i11++) {
            writer.c(WireFormat.getTagFieldNumber(this.tags[i11]), this.objects[i11]);
        }
    }
